...
const StudentUniversities = () => {
  ...
  if (loading) {
    return (
      <div className='loading flex items-center justify-center gap-3 p-8 text-lg text-[color:var(--text-secondary)]'>
          ...
          <div className='grid grid-cols-1 gap-3 md:gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {universities.map((uni, index) => (
              <div
                ...
                className='cursor-pointer rounded-lg border border-[color:var(--border-color)] bg-[color:var(--bg-tertiary)] p-4 transition-all duration-200 ease-in-out hover:translate-y-[-2px] hover:shadow-md hover:border-[color:var(--primary)]'
              >
                <div className='mb-2 flex items-center gap-2'>
                  <FaUniversity className='text-lg text-[color:var(--primary)]' />
