...
const UniversityDetailModal = ({
   ...
   const brandColor = type === 'negeri'
      ? 'text-amber-500'
      : type === 'kedinasan'
         ? 'text-emerald-500'
         : 'text-pink-500';

   const brandBg = type === 'negeri'
      ? 'bg-amber-500/10'
      : type === 'kedinasan'
         ? 'bg-emerald-500/10'
         : 'bg-pink-500/10';

   const brandBorder = type === 'negeri'
      ? 'border-amber-500/20'
      : type === 'kedinasan'
         ? 'border-emerald-500/20'
         : 'border-pink-500/20';

   const headerGradient = type === 'negeri'
      ? 'from-amber-600 to-amber-700'
      : type === 'kedinasan'
         ? 'from-emerald-600 to-emerald-700'
         : 'from-pink-600 to-pink-700';

   const buttonBg = type === 'negeri'
      ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-600/30'
      : type === 'kedinasan'
         ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/30'
         : 'bg-pink-600 hover:bg-pink-700 shadow-pink-600/30';

   const avatarGradient = type === 'negeri'
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                     ...
                     <div className='space-y-6'>
                        <div className='grid grid-cols-1 gap-4'>
                           <div className={`p-6 rounded-3xl ${brandBg} border ${brandBorder} flex items-center justify-between`}>
                              <div>
                                 <p className='text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-1'>Jumlah Alumni</p>
                                 <p className={`text-3xl font-black ${brandColor}`}>{university.count}</p>
                              </div>
                              <div className={`w-12 h-12 rounded-2xl ${brandBg} flex items-center justify-center`}>
                           ...
                           {university.alumni.slice(0, 4).map((alum, idx) => (
                              <div key={idx} className={`flex items-center gap-3 p-3 rounded-2xl border border-slate-200/60 bg-slate-50/50 hover:${brandBorder} transition-colors`}>
                                 ...
                                 <div className='min-w-0'>
                                    ...
                                    <p className={`text-[9px] font-black ${brandColor} uppercase mt-1`}>Angkatan {alum.graduationYear}</p>
                                 </div>
                              </div>
                           ))}
                        </div>
...
const StudentUniversities = () => {
   ...
   const fetchUniversities = async () => {
      setLoading(true);
      try {
         const response = await axios.get<UniversityAggregate[]>('/api/student/universities');
   ...
   const filteredUniversities = universities.filter((uni) => {
      const matchesSearch = uni._id?.name?.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesType = filterType === 'all' || (uni._id?.type || 'swasta') === filterType;
      ...
      <div className='p-4 md:p-8 page-fade-in space-y-6'>
         <div className='mb-6'>
            ...
            <p className='text-[color:var(--text-secondary)] text-sm md:text-base mt-1'>
               Daftar perguruan tinggi tempat alumni melanjutkan studi dan statistikanya.
            </p>
         </div>
            ...
            <Card className='bg-[color:var(--bg-card)] border border-[color:var(--border-color)] shadow-sm'>
               <div className='flex items-center gap-4'>
                  ...
                  <div>
                     ...
                     <h3 className='text-2xl font-black text-[color:var(--text-primary)]'>
                        {universities.filter((u) => !u._id.type || u._id.type === 'swasta').length}
                     </h3>
                  </div>
               </div>
            </Card>
