  ...
  return (
    <div className='p-4 md:p-8 animate-fade-in'>
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8'>
        <div>
          <h1 className='text-lg md:text-2xl font-bold text-[color:var(--text-primary)] !mb-1'>
            Kelola Loker Saya
          </h1>
          <p className='text-[color:var(--text-secondary)] text-xs md:text-sm'>
            Pantau status moderasi dan kelola lowongan aktif Anda
          </p>
        </div>
        <Link
          to='/alumni/jobs/new'
          ...
          <table className='w-full text-left border-collapse'>
            <thead>
              <tr className='bg-[color:var(--bg-tertiary)] border-b border-[color:var(--border-color)]'>
                <th className='px-6 py-4 text-xs font-bold text-[color:var(--text-secondary)] uppercase tracking-wider'>
                  Lowongan
                </th>
                <th className='px-6 py-4 text-xs font-bold text-[color:var(--text-secondary)] uppercase tracking-wider'>
                  Status
                ...
                <th className='px-6 py-4 text-xs font-bold text-[color:var(--text-secondary)] uppercase tracking-wider text-right'>
                  Aksi
                </th>
              </tr>
            </thead>
            <tbody className='divide-y divide-[color:var(--border-color)]'>
              {jobs.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <td className='px-6 py-4'>
                        <div className='font-bold text-[color:var(--text-primary)] text-sm md:text-base'>
                          {job.title}
                        </div>
                      ...
                      <td className='px-6 py-4'>
                        <div className='space-y-1'>
                          <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold ${status.color}`}
                          >
                            <StatusIcon />
                            {status.label}
                          </span>
                ...
                <tr>
                  <td
                    colSpan={4}
                    className='px-6 py-12 text-center text-[color:var(--text-tertiary)] italic text-sm'
                  >
