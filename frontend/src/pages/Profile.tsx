...
import { FaUser, FaLock, FaEnvelope, FaIdBadge, FaSave, FaEyeSlash, FaEye } from 'react-icons/fa';
...
const Profile = () => {
    ...
    const handleUpdateProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            Toast(error.response?.data?.message || 'Gagal memperbarui profil', 'error');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <div className='p-4 md:p-8 animate-fade-in'>
            <div className='mb-8'>
                <h1 className='text-2xl md:text-3xl font-bold text-[color:var(--text-primary)] mb-2'>Profil Pengguna</h1>
                ...
                <div className='bg-[color:var(--bg-card)] rounded-2xl border border-[color:var(--border-color)] overflow-hidden shadow-sm'>
                    <div className='p-6 border-b border-[color:var(--border-color)] bg-[color:var(--bg-tertiary)]/30'>
                        <div className='flex items-center gap-3'>
                            ...
                            <h2 className='text-lg font-bold text-[color:var(--text-primary)] !mb-0'>Data Diri</h2>
                        </div>
                    </div>
                    ...
                    <div className='p-6 border-b border-[color:var(--border-color)] bg-[color:var(--bg-tertiary)]/30'>
                        <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
                            <div className='flex items-center gap-3'>
                                ...
                                <h2 className='text-lg font-bold text-[color:var(--text-primary)] !mb-0'>Ubah Password</h2>
                            </div>
                            <button
                                type='button'
                                onClick={() => {
                    ...
                    {showPasswordFields && (
                        <div className='p-6 grid gap-6 md:grid-cols-2 animate-slide-down'>
                            <div className='space-y-2 relative'>
                                <label className='text-sm font-semibold text-[color:var(--text-secondary)]'>Password Baru</label>
                            ...
                            <div className='space-y-2 relative'>
                                <label className='text-sm font-semibold text-[color:var(--text-secondary)]'>Konfirmasi Password</label>
                                <div className='absolute top-6 inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-[color:var(--text-tertiary)]'>
                                    <FaLock />
                                </div>
                                <input
