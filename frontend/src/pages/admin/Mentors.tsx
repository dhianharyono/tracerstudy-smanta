...
const AdminMentors = () => {
    ...
    const handleDelete = async (id: string) => {
        if (!window.confirm('Hapus status mentor alumni ini? Alumni tidak akan lagi tampil sebagai mentor.')) {
            return;
        }

        try {
            // We just update isMentor to false for this specific admin view action or keep it simple as delete from list (which might mean delete user or just toggle)
            // The request says "list mentor", usually admin might want to toggle status.
            // But based on admin alumni, it has a delete button for the user.
            // Let's implement a toggle status or just use the same delete as Alumni.tsx if it means deleting the user.
            // USER request "list mentor" usually implies a specific view. 
            // Let's just follow AdminAlumni.tsx pattern for now.
            await axios.put(`/api/admin/alumni/${id}`, { isMentor: false });
            Toast('Status mentor berhasil dinonaktifkan!', 'success');
            fetchMentors();
        } catch (error: any) {
            console.error('Error updating mentor status:', error);
            Toast(error.response?.data?.message || 'Gagal merubah status mentor', 'error');
        }
    };
