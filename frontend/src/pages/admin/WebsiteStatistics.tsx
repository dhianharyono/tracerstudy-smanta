        ...
        if (!menuName || menuName === 'Unknown') {
            if (path.includes('/student')) {
                if (path.includes('/student/profile')) {
                    return <span className='text-[color:var(--text-primary)]'>Profile</span>;
                }
                if (path.includes('/student/alumni')) {
                    return <span className='text-[color:var(--text-primary)]'>Alumni</span>;
                }
                if (path.includes('/student/universities')) {
                    return <span className='text-[color:var(--text-primary)]'>Perguruan Tinggi</span>;
                }
                if (path.includes('/student/college-plan')) {
                    return <span className='text-[color:var(--text-primary)]'>Rencana Kuliah</span>;
                }
                if (path.includes('/student/majors')) {
                    return <span className='text-[color:var(--text-primary)]'>Jurusan</span>;
                }
                if (path.includes('/student/events')) {
                    return <span className='text-[color:var(--text-primary)]'>Berita Terkini</span>;
                }
                if (path.includes('/student/news')) {
                    return <span className='text-[color:var(--text-primary)]'>Berita</span>;
                }
                return <span className='text-[color:var(--text-primary)]'>Dashboard Siswa</span>;
            }
            if (path.includes('/alumni')) {
                ...
                if (path.includes('/alumni/questionnaire')) {
                    return <span className='text-[color:var(--text-primary)]'>Kuesioner</span>;
                }
                if (path.includes('/alumni/mutual-alumni')) {
                    return <span className='text-[color:var(--text-primary)]'>Rekan Seangkatan</span>;
                }
                ...
                return <span className='text-[color:var(--text-primary)]'>Dashboard Alumni</span>;
            }
        }
        return menuName;
    };
        ...
        <div className='p-6 page-fade-in'>
                        ...
                        <div className='flex items-center gap-2 mb-1'>
                            ...
                            <div className='group relative'>
                                ...
                                <div className='pointer-events-none absolute bottom-full left-1/2 mb-2 w-48 -translate-x-1/2 rounded bg-gray-800 p-2 text-center text-xs text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100 z-50'>
                                    Menghitung setiap kali halaman dibuka (Page View), termasuk refresh halaman.
                ...
                <div className='bg-[color:var(--bg-card)] p-6 rounded-xl border border-[color:var(--border-color)] shadow-sm flex items-center gap-4'>
                    ...
                    <div>
                        ...
                        <h3 className='text-lg font-bold text-[color:var(--text-primary)] truncate max-w-[250px]'>
                            {
                                stats?.popularPages?.[0]?.path ? renderMenuName('', stats?.popularPages?.[0]?.path) : '-'
                            }
                        </h3>
                    </div>
                </div>
                        ...
                        <ResponsiveContainer width='100%' height='100%'>
                            <PieChart>
                                <Pie
                                    ...
                                    dataKey='count'
                                    nameKey='_id'
                                    label={({ name, percent }) => `${name === 'student' ? 'Siswa' : 'Alumni'} ${(percent * 100).toFixed(0)}%`}
                                >
                                ...
                                <Tooltip />
                                <Legend
                                    formatter={(value) => value === 'student' ? 'Siswa' : 'Alumni'}
                                />
                            </PieChart>
                        </ResponsiveContainer>
                    </div>
            ...
            <div className='bg-[color:var(--bg-card)] rounded-xl border border-[color:var(--border-color)] shadow-sm overflow-hidden'>
                <div className='p-6 border-b border-[color:var(--border-color)]'>
                    ...
                    <p className='mt-1 text-xs text-[color:var(--text-tertiary)] flex items-center gap-2'>
                        <FaInfoCircle className='shrink-0' />
                        <span>
                            <strong>Total Kunjungan:</strong> Jumlah total halaman dibuka (pageviews).
                            <span className='mx-2'>•</span>
                            <strong>Pengunjung Unik:</strong> Jumlah pengguna berbeda (distinct users) yang membuka halaman.
                        </span>
                    </p>
                </div>
                    ...
                    <table className='w-full text-left text-sm'>
                        <thead className='bg-[color:var(--bg-tertiary)] text-[color:var(--text-secondary)] uppercase font-medium'>
                            <tr>
                                ...
                                <th className='px-6 py-4 cursor-help' title='Total kali halaman ini dimuat'>Total Kunjungan</th>
                                <th className='px-6 py-4 cursor-help' title='Jumlah user berbeda yang mengunjungi halaman ini'>Pengunjung Unik</th>
                            </tr>
                        </thead>
                        <tbody className='divide-y divide-[color:var(--border-color)]'>
                            {stats?.popularPages?.map((page: any, index: number) => (
                                <tr key={index} className='hover:bg-[color:var(--bg-tertiary)]/50'>
                                    ...
                                    <td className={`px-6 py-4 font-medium text-[color:var(--text-tertiary)] ${page.path.includes('/student') ? 'text-blue-300' : 'text-green-300'}`}>
                            ...
                            {(!stats?.popularPages || stats.popularPages.length === 0) && (
                                <tr>
                                    <td colSpan={5} className='px-6 py-8 text-center text-[color:var(--text-tertiary)]'>
