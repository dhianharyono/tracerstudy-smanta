...
const AdminOpportunityManagement = () => {
  const [activeTab, setActiveTab] = useState<'moderation' | 'view'>('view');
  ...
  return (
    <div className="p-4 md:p-8 animate-fade-in">
      <div className="mb-8">
        <h1 className="text-xl md:text-3xl font-bold text-[color:var(--text-primary)] !mb-1">Bursa Kerja</h1>
        <p className="text-[color:var(--text-secondary)] text-sm">Manajemen dan pemantauan bursa kerja alumni</p>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-[color:var(--border-color)] mb-8">
        <button
          onClick={() => setActiveTab('view')}
          className={`flex items-center gap-2 px-6 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'view'
            ? 'border-[var(--primary)] text-[var(--primary)]'
            : 'border-transparent text-[color:var(--text-tertiary)] hover:text-[color:var(--text-secondary)]'
            }`}
        >
          <FaGlobe className="text-xs" /> Daftar Bursa Kerja
        </button>
        <button
          onClick={() => setActiveTab('moderation')}
          className={`flex items-center gap-2 px-6 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'moderation'
            ? 'border-[var(--primary)] text-[var(--primary)]'
            : 'border-transparent text-[color:var(--text-tertiary)] hover:text-[color:var(--text-secondary)]'
            }`}
        >
          <FaShieldAlt className="text-xs" /> Moderasi Bursa Kerja
        </button>
      </div>
          ...
          <div className="flex gap-4 mb-6 overflow-x-auto pb-2 scrollbar-hide">
            {['pending', 'approved', 'rejected', 'closed'].map(status => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-bold capitalize transition-all whitespace-nowrap ${filterStatus === status
