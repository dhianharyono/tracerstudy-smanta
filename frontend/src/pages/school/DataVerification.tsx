...
const SchoolDataVerification = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('/api/school/alumni/verify-bulk', formData, {
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-8'>
        <Card className='!bg-blue-50 dark:!bg-blue-900/10 border-blue-100 dark:border-blue-900/30'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-sm text-blue-600 dark:text-blue-400 font-medium'>Total Alumni Terdaftar</p>
              <h3 className='text-2xl font-bold text-blue-900 dark:text-blue-100'>{stats?.totalAlumni || 0}</h3>
            </div>
          </div>
        </Card>
        <Card className='!bg-green-50 dark:!bg-green-900/10 border-green-100 dark:border-green-900/30'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-sm text-green-600 dark:text-green-400 font-medium'>Sudah Mengisi Kuesioner</p>
              <h3 className='text-2xl font-bold text-green-900 dark:text-green-100'>{stats?.completedAlumni || 0}</h3>
            </div>
          </div>
        </Card>
        <Card className='!bg-purple-50 dark:!bg-purple-900/10 border-purple-100 dark:border-purple-900/30'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <p className='text-sm text-purple-600 dark:text-purple-400 font-medium'>Progress Sinkronisasi</p>
              <h3 className='text-2xl font-bold text-purple-900 dark:text-purple-100'>
                {stats?.totalAlumni ? Math.round(((stats?.verifiedAlumni || 0) / stats.totalAlumni) * 100) : 0}%
              </h3>
            </div>
          </div>
        </Card>
            ...
            {!isBK ? (
              <div className='p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/30 rounded-xl'>
                <p className='text-sm text-amber-700 dark:text-amber-400 flex items-start gap-2'>
                  ...
                  Hanya Guru BK yang memiliki otoritas untuk mengunggah dan mensinkronisasikan data alumni.
                </p>
              </div>
            ) : (
              <div className='space-y-6'>
                <div>
                  <p className='text-sm text-[color:var(--text-secondary)] mb-4'>
                    Gunakan template standar kami untuk memastikan data terbaca dengan benar oleh sistem.
                  </p>
                  <button
                ...
                <div className='my-6 border-t border-[color:var(--border-color)] pt-6'>
                  <label className='block text-sm font-semibold mb-2'>Pilih File Data BK</label>
                  <input
                    type='file'
                    accept='.xlsx, .xls'
                  ...
                  />
                  <p className='text-[10px] text-gray-400 mt-2'>Format: .xlsx atau .xls</p>
                </div>
        ...
        <div className='lg:col-span-2'>
          {!results ? (
            <div className='h-full flex flex-col items-center justify-center p-12 text-center opacity-60'>
              ...
              <p className='max-w-xs'>Silakan unggah file data sekolah untuk melihat hasil sinkronisasi dengan data kuesioner aplikasi.</p>
            </div>
          ) : (
            <div className='space-y-6'>
              {/* Summary Results */}
              <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
                <div className='bg-green-500/10 border border-green-500/20 p-4 rounded-xl text-center'>
                  <p className='text-xs font-bold text-green-600 uppercase mb-1'>Cocok & Terverifikasi</p>
                  <p className='text-2xl font-black text-green-700'>{results.summary.verifiedCount}</p>
                </div>
                <div className='bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl text-center'>
                  <p className='text-xs font-bold text-amber-600 uppercase mb-1'>Data Berbeda (Mismatch)</p>
                  <p className='text-2xl font-black text-amber-700'>{results.summary.mismatchCount}</p>
                </div>
                <div className='bg-red-500/10 border border-red-500/20 p-4 rounded-xl text-center'>
                  <p className='text-xs font-bold text-red-600 uppercase mb-1'>Belum Terdaftar (Missing)</p>
                  <p className='text-2xl font-black text-red-700'>{results.summary.notFoundCount}</p>
                </div>
              </div>
                      ...
                      <table className='w-full text-sm text-left'>
                        <thead className='text-xs bg-gray-50 dark:bg-gray-800 uppercase'>
                          <tr>
                            ...
                            <th className='px-4 py-2 text-gray-500'>Data Apps (Tracer)</th>
                            <th className='px-4 py-2 text-gray-500'>Data Sekolah (BK)</th>
                          </tr>
                        </thead>
                        <tbody className='divide-y divide-[color:var(--border-color)]'>
                          {results.details.mismatch.map((m: any, i: number) => (
                            <tr key={i} className='hover:bg-gray-50/50 dark:hover:bg-gray-800/30'>
                              ...
                              <td className='px-4 py-3 text-gray-400'>{m.gradYear}</td>
                              <td className='px-4 py-3'>
                                <span className='text-xs py-0.5 px-2 bg-blue-100 text-blue-700 rounded-full'>{m.dbData.status}</span>
                                <p className='text-[10px] mt-1 truncate max-w-[150px]'>{m.dbData.university || m.dbData.institution}</p>
                              </td>
                              <td className='px-4 py-3'>
                                <span className='text-xs py-0.5 px-2 bg-orange-100 text-orange-700 rounded-full font-bold'>{m.excelData.status}</span>
                                <p className='text-[10px] mt-1 truncate max-w-[150px]'>{m.excelData.university || m.excelData.institution}</p>
                              </td>
                            </tr>
                          ))}
                        </tbody>
                ...
                {results.details.notFound.length > 0 && (
                  <Card>
                    <div className='flex items-center justify-between mb-4'>
                      ...
                      <p className='text-[10px] text-gray-400 italic'>Silakan undang mereka untuk bergabung</p>
                    </div>
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-3'>
                      {results.details.notFound.map((n: any, i: number) => (
                        <div key={i} className='flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700'>
                          <div>
                            <p className='text-sm font-bold'>{n.name}</p>
                            <p className='text-xs text-gray-500'>Angkatan {n.gradYear}</p>
                          </div>
