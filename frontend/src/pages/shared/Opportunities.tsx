  ...
  return (
    <div className="p-4 md:p-8 animate-fade-in">
      <div className="mb-8 text-center md:text-left">
        <h1 className="text-xl md:text-3xl font-bold text-[color:var(--text-primary)] !mb-2 flex items-center justify-center md:justify-start gap-3">
      ...
      <div className="space-y-4">
        {jobs.length > 0 ? (
          jobs.map((job) => {
            ...
            return (
              ...
              >
                <div className="p-4 md:p-6 flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[color:var(--text-tertiary)]">
                      ...
                      <div className="flex items-center gap-1.5">
                        <FaClock />
                        <span>Batas: {new Date(job.expiryDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 md:mt-0 flex items-center justify-between md:justify-end gap-3 border-t md:border-0 pt-4 md:pt-0">
                    <div className="text-[10px] text-[color:var(--text-tertiary)] md:hidden">
                      Diposting oleh: <span className="font-semibold text-[color:var(--text-secondary)]">{job.postedBy?.profile?.fullName || 'Alumni'}</span>
                    </div>
                ...
                {/* Expandable Content */}
                <div className={`px-6 overflow-hidden transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[2000px] py-6 border-t border-[color:var(--border-color)] bg-[color:var(--bg-tertiary)]/20' : 'max-h-0'}`}>
                  <div className="ml-0 md:ml-16 space-y-6">
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-[color:var(--text-tertiary)] mb-2">Deskripsi Pekerjaan</h4>
                    ...
                    {job.requirements && job.requirements.length > 0 && (
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-wider text-[color:var(--text-tertiary)] mb-2">Persyaratan</h4>
                    ...
                    <div className="pt-4 border-t border-[color:var(--border-color)] flex items-center justify-between">
                      <div className="text-xs text-[color:var(--text-tertiary)]">
                        Pemberi Loker: <span className="text-[color:var(--text-secondary)] font-bold">{job.postedBy?.profile?.fullName}</span>
                        <span className="mx-2">•</span>
                        Angkatan: <span className="text-[color:var(--text-secondary)] font-bold">{job.postedBy?.profile?.graduationYear}</span>
                      </div>

                      {job.applicationLink && (
                        <div className="text-[10px] text-[color:var(--text-tertiary)] italic">
                          Klik 'Lamar Sekarang' untuk diarahkan ke web eksternal.
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {!isExpanded && (
                  <div className="hidden md:block px-6 pb-4 ml-20">
                    <p className="text-xs text-[color:var(--text-tertiary)] line-clamp-1 italic cursor-pointer" onClick={() => toggleExpand(job._id)}>
                      "{job.description.split('\n')[0]}..." <span className="text-[var(--primary)] font-bold">baca selengkapnya</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="text-center py-12 bg-[color:var(--bg-card)] rounded-2xl border border-dashed border-[color:var(--border-color)]">
            ...
            <h3 className="text-lg font-medium text-[color:var(--text-primary)]">Belum ada lowongan</h3>
            <p className="text-xs text-[color:var(--text-tertiary)]">Coba cari kategori lain atau kembali lagi nanti.</p>
          </div>
        )}
      </div>
    </div>
