    ...
    <div className='card flex flex-col h-full'>
      ...
      {data.length > 0 ? (
        <div className='flex flex-col flex-1 gap-0'>
          {data.slice(0, 3).map((newsItem: NewsData, index: number) => (
            <div
              key={newsItem._id}
              onClick={() =>
                navigate(
              ...
              className={`cursor-pointer py-4 border-b border-[color:var(--border-color)] last:border-0 hover:bg-[color:var(--bg-card-hover)] transition-colors -mx-2 px-2 rounded-lg group
                ${index === 0 ? 'bg-slate-700 animate-[pulse_2s_ease-in-out_5]' : ''}`}
            >
              <h3 className='mb-1.5 text-xs font-bold text-[color:var(--text-primary)] group-hover:text-[color:var(--primary)] transition-colors leading-snug line-clamp-2'>
                {newsItem.title}
              </h3>
              <p className='text-[10px] md:text-xs text-[color:var(--text-secondary)] line-clamp-2 mb-2 leading-relaxed'>
